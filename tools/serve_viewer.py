...
Collects every `run.json` under the given roots into `viewer/runs/`, then serves
...
def make_handler(roots):
    class Handler(http.server.SimpleHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if self.path.rstrip("/").endswith("/viewer/runs/index.json"):
                try:
...
def main(argv=None):
    ...
    print("open      http://localhost:%d/viewer/" % args.port)
